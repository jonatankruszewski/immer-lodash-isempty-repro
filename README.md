...
[`48fc378`](https://github.com/immerjs/immer/commit/48fc378), which wrapped both
`constructor` and `__proto__` reads in a sanitizing `Proxy`. On a draft
`draft.constructor` **is** the real `Object`, whose `prototype` is a
non-configurable data property, so a wrapping Proxy that hides it violates a
Proxy invariant and **throws** — the crash above.
